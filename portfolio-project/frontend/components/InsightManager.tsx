...
const InsightManager: React.FC<InsightManagerProps> = ({ token }) => {
  ...
  const fetchInsights = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:8081/api/insights/all', {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingInsight 
        ? `http://localhost:8081/api/insights/${editingInsight.id}`
        : 'http://localhost:8081/api/insights';
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:8081/api/insights/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const togglePublish = async (id: number) => {
    try {
      const response = await fetch(`http://localhost:8081/api/insights/${id}/publish`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
